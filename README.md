...
primitive over an anti-detect browser and stop there. BABA_Always_Bypasses_Anti-bot is built on
...
- **Log in by hand once, the agent reuses it forever.** Some sites need a
  ...
  BABA_Always_Bypasses_Anti-bot runs a live web view of the browser (`http://localhost:6080/vnc.html`)
...
- **Lazy auto-start, zero-config install.** `pi install npm:baba_always_bypasses_anti-bot` and
