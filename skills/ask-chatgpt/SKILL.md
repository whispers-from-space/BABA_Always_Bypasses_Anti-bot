---
name: ask-chatgpt
description: Relays a question to ChatGPT and returns its exact response. Use only when the user explicitly asks to consult, check with, or get ChatGPT's answer/opinion specifically — not for general questions you can answer yourself.
disable-model-invocation: true
---

# Ask ChatGPT

Automates the ChatGPT web UI via the shared camofox-browser server. Invoked as
`/skill:ask-chatgpt` — never self-triggered, since `disable-model-invocation`
...
1. Read that message, plus any conversation context needed to phrase a
   complete, self-contained question (ChatGPT has no access to this
...
```bash
node ask-chatgpt.js "the fully-phrased question here"
# or: echo "question" | node ask-chatgpt.js
```
...
| `no response received from ChatGPT` | ChatGPT didn't answer in time | ChatGPT didn't produce a response |
...
conversation context) as the `question` argument to ask_chatgpt, and
